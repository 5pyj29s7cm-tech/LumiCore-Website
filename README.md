...
首页使用约 4 分 07 秒的 1920×1080 完整录屏。视频位于
`assets/lumi-client-command-center.mp4`，封面为 `assets/lumi-client-poster.webp`。
MP4 保留原视频与音频流，并将播放索引放在文件开头，以便尽快开始播放。
播放器按需加载，支持进度跳转；离开首页、滚出画面或切换后台时暂停。
更新媒体、样式或脚本时，请同时更新 `index.html` 中对应的版本参数，避免缓存旧内容。
